...
const Garage = {
    ...
    loadCar(index) {
        ...
        const createFallbackCar = () => {
            ...
            const body = new THREE.Mesh(new THREE.BoxGeometry(2, 0.8, 4), new THREE.MeshStandardMaterial({ color: 0xff0044, metalness: 0.8, roughness: 0.2 }));
            body.position.y = 0.6;
            group.add(body);
            const cabin = new THREE.Mesh(new THREE.BoxGeometry(1.5, 0.6, 1.8), new THREE.MeshStandardMaterial({ color: 0x333333, metalness: 0.9, roughness: 0.1 }));
        ...
        loader.load(car.file, gltf => {
            ...
            console.log('GLB load failed, using fallback car');
            State.garageCar = createFallbackCar();
            State.garageScene.add(State.garageCar);
            this.updateUI(index);
        });
    },
...
const Game = {
    ...
    createFallbackCar() {
        const group = new THREE.Group();
        const body = new THREE.Mesh(new THREE.BoxGeometry(2, 0.8, 4), new THREE.MeshStandardMaterial({ color: 0xff0044, metalness: 0.6, roughness: 0.3 }));
        body.position.y = 0.5;
        body.castShadow = true;
        group.add(body);
        const cabin = new THREE.Mesh(new THREE.BoxGeometry(1.5, 0.6, 1.8), new THREE.MeshStandardMaterial({ color: 0x222222 }));
    ...
    async loadCar() {
        ...
        return new Promise(resolve => {
            ...
            // Timeout after 5 seconds - use fallback car
            const timeout = setTimeout(() => {
                console.log('GLB load timeout, using fallback car');
                if (!State.carModel) {
                    State.carModel = this.createFallbackCar();
            ...
            (xhr) => {
                ...
                progress(40 + percent * 0.4, `Loading car... ${percent}%`);
            },
            () => {
                clearTimeout(timeout);
                console.log('GLB load failed, using fallback car');
